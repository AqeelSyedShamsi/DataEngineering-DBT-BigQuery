...
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_video_thruplay_watched_actions') }} thruplay_watched_actions
    ON ads.ad_id = thruplay_watched_actions.ad_id AND ads.`date` = thruplay_watched_actions.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_video_30_sec_watched_actions') }} video_30s
