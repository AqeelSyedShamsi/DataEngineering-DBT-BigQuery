...
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_p_50_watched_actions') }} p_50_watched
    ON ads.ad_id = p_50_watched.ad_id AND ads.`date` = p_50_watched.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_p_75_watched_actions') }} p_75_watched
    ON ads.ad_id = p_75_watched.ad_id AND ads.`date` = p_75_watched.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_p_95_watched_actions') }} p_95_watched
    ON ads.ad_id = p_95_watched.ad_id AND ads.`date` = p_95_watched.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_p_100_watched_actions') }} p_100_watched
    ON ads.ad_id = p_100_watched.ad_id AND ads.`date` = p_100_watched.`date`
LEFT JOIN {{ source('bigquery_nt_fb', 'device_breakdown_outbound_clicks') }} out_click
