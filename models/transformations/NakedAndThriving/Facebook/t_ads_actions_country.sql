...
JOIN
    {{ ref('raw_country_breakdown_actions') }}  actions
    ON ads.ad_id = actions.ad_id AND ads.`date` = actions.`date`
JOIN
    {{ ref('raw_country_breakdown_action_values') }} actions_values
    ON ads.ad_id = actions_values.ad_id AND ads.`date` = actions_values.`date`
JOIN
    {{ ref('raw_country_breakdown_cost_per_unique_action_type') }} cost_per_unique_action
    ON ads.ad_id = cost_per_unique_action.ad_id AND ads.`date` = cost_per_unique_action.`date`
JOIN
    {{ ref('raw_country_breakdown_cost_per_action_type') }} cost_per_action_type
