...
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_cost_per_unique_outbound_click') }} cost_per_unique_out_click
    ON ads.ad_id = cost_per_unique_out_click.ad_id AND ads.`date` = cost_per_unique_out_click.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_website_ctr') }} website_ctr
...
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_video_continuous_2_sec_watched_actions') }} watched_actions
