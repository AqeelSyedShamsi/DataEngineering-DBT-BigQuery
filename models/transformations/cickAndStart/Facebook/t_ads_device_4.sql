...
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_thruplay_watched_actions') }} thruplay_watched_actions
    ON ads.ad_id = thruplay_watched_actions.ad_id AND ads.`date` = thruplay_watched_actions.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_30_sec_watched_actions') }} video_30s
    ON ads.ad_id = video_30s.ad_id AND ads.`date` = video_30s.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_avg_time_watched_actions') }} avg_time_watched
    ON ads.ad_id = avg_time_watched.ad_id AND ads.`date` = avg_time_watched.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'device_breakdown_video_p_25_watched_actions') }} p_25_watched
