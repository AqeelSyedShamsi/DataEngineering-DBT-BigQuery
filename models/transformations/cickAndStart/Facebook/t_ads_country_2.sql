...
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_website_purchase_roas') }} purchase_roas
    ON ads.ad_id = purchase_roas.ad_id AND ads.`date` = purchase_roas.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_outbound_clicks_ctr') }} out_click_ctr
    ON ads.ad_id = out_click_ctr.ad_id AND ads.`date` = out_click_ctr.`date`
LEFT JOIN
    {{ source('bigquery_nt_fb', 'country_breakdown_cost_per_outbound_click') }} cost_per_out_conv
